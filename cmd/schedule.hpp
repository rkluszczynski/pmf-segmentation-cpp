...
struct classcomp
{
    classcomp(double eps) : epsilon(eps) {}
    ...
    bool operator() (const EVENT & e1, const EVENT & e2) const
    {
            ...
            cout << "[]  " << e1->GetPoint()->x << " < " << e2->GetPoint()->x << "  " << (res ? "TRUE" : "FALSE") << endl;
            // */
            if (! IsZero(e1->GetPoint()->x - e2->GetPoint()->x, epsilon))
            {
                if (e1->GetPoint()->x < e2->GetPoint()->x) return true;
                if (e1->GetPoint()->x > e2->GetPoint()->x) return false;
            }
            ...
            if (! IsZero(e1->GetPoint()->y - e2->GetPoint()->y, epsilon))
            {
                if (e1->GetPoint()->y < e2->GetPoint()->y) return true;
                if (e1->GetPoint()->y > e2->GetPoint()->y) return false;
            }
    ...
    class EventsSchedule
    {
        ...
        typedef std::set<EventPoint, classcomp<EventPoint> > EventList;

        public:
            typedef typename EventList::iterator Iterator;

            EventsSchedule(REAL epsilon)
            {
                _events = new EventList(classcomp<EventPoint>(epsilon));
            };
            ~EventsSchedule() { delete _events; };
    ...
    class EventsSchedule2
    {
        ...
        public:
            ...
            friend ostream & operator << (ostream & out, const EventsSchedule2 * es)
            {
                out << "[ EVENTS ] :>";
                FOREACH(it, *(es->_events))  {
                    POINT * pt = (*it)->GetPoint();
    ...
    bool
    EventsSchedule2<REAL, COMP>::InsertBirthEvent (POINT * pt)
    {
        EventType type = (pt->type == PT_BirthOnBorder) ? BorderBirth : NormalBirth;
        BirthEvent * be = new BirthEvent(pt, type);
        return Insert(be);
