...
class BlocksLists
{
    ...
    public :
		...
		friend std::ostream & operator << (std::ostream & out, const BlocksLists<T_REAL> * blists)
		{
			...
			out << std::endl;
			out << "      horizontal : ";
			for (int i = 0; i < blists->hLimit.size(); i++)  std::cout << " " << blists->hLimit[i];
			out << std::endl;
			out << "        vertical : ";
			for (int i = 0; i < blists->vLimit.size(); i++)  std::cout << " " << blists->vLimit[i];
			out << std::endl;
 ...
 **/
template <class T_REAL>
BlocksLists<T_REAL>::BlocksLists (T_REAL fheight, T_REAL fwidth, T_REAL bsize)
: fieldHeight(fheight), fieldWidth(fwidth), blockHeight(bsize), blockWidth(bsize)
{
